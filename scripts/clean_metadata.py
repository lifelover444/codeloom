...
def main():
    ...
    # Adjust path relative to the script's location in the codeloom repo
    litellm_path = script_dir.parent / "../litellm/model_prices_and_context_window.json"
    codeloom_path = script_dir / "../codeloom/resources/model-metadata.json"

    if not litellm_path.exists():
        print(f"Error: LiteLLM metadata file not found at {litellm_path}")
        return

    if not codeloom_path.exists():
        print(f"Error: Codeloom metadata file not found at {codeloom_path}")
        return
    ...
    try:
        # Use json5 for the codeloom metadata file as it might contain comments
        with open(codeloom_path, "r") as f:
            codeloom_data = json5.load(f)
    except json.JSONDecodeError as e:
        print(f"Error decoding JSON from {codeloom_path}: {e}")
        return
    except Exception as e:
        print(f"Error reading {codeloom_path}: {e}")
        return

    litellm_keys = set(litellm_data.keys())
    codeloom_keys = set(codeloom_data.keys())

    common_keys = sorted(list(litellm_keys.intersection(codeloom_keys)))
    removed_count = 0

    if common_keys:
        print("Comparing common models found in both files:\n")
        for key in common_keys:
            print(f"--- {key} (codeloom) ---")
            print(f"+++ {key} (litellm) +++")

            litellm_entry = litellm_data.get(key, {})
            codeloom_entry = codeloom_data.get(key, {})

            # Convert dicts to formatted JSON strings for comparison
            # First, compare the dictionaries directly for semantic equality
            if litellm_entry == codeloom_entry:
            ...
            codeloom_entry_copy = codeloom_entry.copy()
            dummy_key = "zzzdummykey"
            litellm_entry_copy[dummy_key] = True
            codeloom_entry_copy[dummy_key] = True

            litellm_json_lines = json.dumps(
                litellm_entry_copy, indent=4, sort_keys=True
            ).splitlines()
            codeloom_json_lines = json.dumps(codeloom_entry_copy, indent=4, sort_keys=True).splitlines()
            ...
            codeloom_json_filtered = [line for line in codeloom_json_lines if dummy_key not in line]

            diff = difflib.unified_diff(
                codeloom_json_filtered,
                litellm_json_filtered,
                fromfile=f"{key} (codeloom)",
                tofile=f"{key} (litellm)",
                lineterm="",
                n=max(len(litellm_json_filtered), len(codeloom_json_filtered)),  # Show all lines
            )
            ...
            # Ask user if they want to remove the entry from codeloom's metadata
            response = (
                input(f"Remove '{key}' from codeloom/resources/model-metadata.json? (y/N): ")
                .strip()
                .lower()
            )
            if response == "y":
                # Perform surgical removal from the text file
                if remove_block_surgically(codeloom_path, key):
                    removed_count += 1
                    # Optional: Also remove from the in-memory dict if needed later,
                    # but it's not strictly necessary if we reload or finish now.
                    # if key in codeloom_data: del codeloom_data[key]
                else:
                    print(f"Failed to remove '{key}' block surgically.")
                    # Key might still be in codeloom_data if removal failed
            else:
                print(f"Keeping '{key}'.")
            print("-" * 40 + "\n")  # Separator for the next model
