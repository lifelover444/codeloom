...
website_files = [
    ...
    "codeloom/website/share/index.md",
    "codeloom/website/_includes/head_custom.html",
    "codeloom/website/_includes/home.css",
    "codeloom/website/docs/leaderboards/index.md",
]

exclude_files = [
    "codeloom/website/install.ps1",
    "codeloom/website/install.sh",
]
...
def blame(start_tag, end_tag=None):
    ...
    files = [
        ...
        or (f.startswith("codeloom/resources/") and f.endswith(".yml"))
    ...
    for file in files:
        ...
        if file_counts:
            ...
            for author, count in file_counts.items():
                ...
                if "(codeloom)" in author.lower():
                    codeloom_total += count

    total_lines = sum(grand_total.values())
    codeloom_percentage = (codeloom_total / total_lines) * 100 if total_lines > 0 else 0

    end_date = get_tag_date(end_tag if end_tag else "HEAD")

    return all_file_counts, grand_total, total_lines, codeloom_total, codeloom_percentage, end_date
...
def get_commit_authors(commits):
    ...
    for commit in commits:
        ...
        if lower_subject.startswith("codeloom:") or "co-authored-by: codeloom" in lower_full:
            author += " (codeloom)"
...
def process_all_tags_since(start_tag):
    ...
    for i in tqdm(range(len(tags) - 1), desc="Processing tags"):
        ...
        all_file_counts, grand_total, total_lines, codeloom_total, codeloom_percentage, end_date = blame(
            start_tag, end_tag
        )
        results.append(
            {
                ...
                "codeloom_total": codeloom_total,
                "codeloom_percentage": round(codeloom_percentage, 2),
            }
        )
    return results
...
def main():
    parser = argparse.ArgumentParser(description="Get codeloom/non-codeloom blame stats")
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Find all tags since the specified tag and print codeloom percentage between each pair of"
            " successive tags"
        ),
    )
    parser.add_argument(
    ...
    if not args.all_since:
        print(f"- Codeloom wrote {round(codeloom_percentage)}% of the code in this release.")
