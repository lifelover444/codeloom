...
def get_latest_version_from_history():
    ...
    match = re.search(r"### Codeloom v(\d+\.\d+\.\d+)", history_content)
    if not match:
...
def main():
    ...
    version_header = f"### Codeloom v{latest_ver}"
    ...
    next_version_idx = history_content.find("\n### Codeloom v", version_idx + len(version_header))
    ...
    # Run blame to get codeloom percentage
    blame_result = subprocess.run(["python3", "scripts/blame.py"], capture_output=True, text=True)
    codeloom_line = blame_result.stdout.strip().split("\n")[-1]  # Get last line with percentage

    # Construct and run the codeloom command
    message = history_prompt.format(codeloom_line=codeloom_line)
    ...
    # Run update-docs.sh after codeloom
