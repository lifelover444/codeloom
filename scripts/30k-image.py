...
# flake8: noqa: E501
"""
Generate a celebratory SVG image for Codeloom reaching 30,000 GitHub stars.
...
CODELOOM_GREEN = "#14b014"
CODELOOM_BLUE = "#4C6EF5"
DARK_COLOR = "#212529"
LIGHT_COLOR = "#F8F9FA"
GOLD_COLOR = "#f1c40f"
...
def embed_font():
    ...
    font_path = (
        Path(__file__).parent.parent / "codeloom" / "website" / "assets" / "Glass_TTY_VT220.ttf"
    )
...
def generate_confetti(count=150, width=DEFAULT_WIDTH, height=DEFAULT_HEIGHT):
    ...
    confetti = []
    colors = [CODELOOM_GREEN, CODELOOM_BLUE, GOLD_COLOR, "#e74c3c", "#9b59b6", "#3498db", "#2ecc71"]
...
<svg width="{width}" height="{height}" viewBox="0 0 {width} {height}" xmlns="http://www.w3.org/2000/svg">
  ...
  <style>
    ...
    .codeloom-logo {{ font-family: 'GlassTTYVT220', monospace; font-size: 120px; fill: {CODELOOM_GREEN}; text-anchor: middle; filter: url(#glow); }}
  ...
  <g clip-path="url(#rounded-rect)">
    ...
    <!-- Main content -->
    <text x="{width/2}" y="{height/2 - 100}" class="codeloom-logo">codeloom</text>
    ...
    <text x="{width/2}" y="{height - 50}" class="footer">github.com/Codeloom-AI/codeloom</text>
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate a celebration SVG for Codeloom's 30K GitHub stars"
    )
    parser.add_argument(
        "--output",
        "-o",
        type=str,
        default="codeloom-30k-stars.svg",
        help="Output file path (default: codeloom-30k-stars.svg)",
    )
    parser.add_argument(
        "--width",
        "-w",
