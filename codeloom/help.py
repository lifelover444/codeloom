...
from codeloom import __version__, utils
from codeloom.dump import dump  # noqa: F401
from codeloom.help_pats import exclude_website_pats
...
def get_package_files():
    for path in importlib_resources.files("codeloom.website").iterdir():
        if path.is_file():
            yield path
        elif path.is_dir():
...
def fname_to_url(filepath):
    ...
    return f"https://codeloom.chat/{url_path}"
...
def get_index():
    ...
    dname = Path.home() / ".codeloom" / "caches" / ("help." + __version__)
    ...
    if index is None:
        ...
        for fname in get_package_files():
            ...
            doc = Document(
                text=importlib_resources.files("codeloom.website")
                .joinpath(fname)
