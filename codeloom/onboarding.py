...
def start_openrouter_oauth_flow(io, analytics):
    ...
    callback_url = f"http://localhost:{port}/callback/codeloom"
    auth_code = None
    server_error = None
    ...
    class OAuthCallbackHandler(http.server.SimpleHTTPRequestHandler):
        def do_GET(self):
            ...
            if parsed_path.path == "/callback/codeloom":
                ...
                if "code" in query_params:
                    ...
                    self.wfile.write(
                        ...
                        b"<p>Codeloom has received the authentication code. "
                    ...
                    # Redirect to codeloom website if 'code' is missing (e.g., user visited manually)
    ...
    io.tool_output("\nPlease open this URL in your browser to connect Codeloom with OpenRouter:")
    io.tool_output()
    print(auth_url)
    ...
    if api_key:
        ...
        try:
            config_dir = os.path.expanduser("~/.codeloom")
            os.makedirs(config_dir, exist_ok=True)
            key_file = os.path.join(config_dir, "oauth-keys.env")
            with open(key_file, "a", encoding="utf-8") as f:
                f.write(f'OPENROUTER_API_KEY="{api_key}"\n')

            io.tool_warning("Codeloom will load the OpenRouter key automatically in future sessions.")
            io.tool_output()
