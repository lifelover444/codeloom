...
from codeloom import __version__, branding
from codeloom.args_formatter import (
    DotEnvFormatter,
    MarkdownHelpFormatter,
    YamlHelpFormatter,
)
from codeloom.deprecated import add_deprecated_model_args
...
def resolve_codeloomignore_path(path_str, git_root=None):
    path = Path(path_str)
    if path.is_absolute():
        return str(path)
    elif git_root:
...
def get_parser(default_config_files, git_root):
    parser = configargparse.ArgumentParser(
        ...
        auto_env_var_prefix=branding.ENV_PREFIX,
    )
    ...
    group.add_argument(
        ...
        default=branding.MODEL_SETTINGS_FILE,
        help=f"Specify a file with {branding.PRODUCT_NAME} model settings for unknown models",
    ).complete = shtab.FILE
    group.add_argument(
        "--model-metadata-file",
        metavar="MODEL_METADATA_FILE",
        default=branding.MODEL_METADATA_FILE,
    ...
    default_input_history_file = (
        branding.repo_file(git_root, branding.INPUT_HISTORY_FILE)
    )
    default_chat_history_file = (
        branding.repo_file(git_root, branding.CHAT_HISTORY_FILE)
    )
    group.add_argument(
        "--input-history-file",
    ...
    group.add_argument(
        ...
        default=None,
        help=(
            "Log the conversation with the LLM to this file "
            f"(for example, {branding.LLM_HISTORY_EXAMPLE})"
        ),
    ).complete = shtab.FILE
    ...
    group.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=True,
        help=f"Enable/disable adding {branding.PRODUCT_NAME_LOWER} files to .gitignore (default: True)",
    )
    group.add_argument(
        "--add-gitignore-files",
        action=argparse.BooleanOptionalAction,
        default=False,
        help=(
            "Enable/disable the addition of files listed in .gitignore to "
            f"{branding.PRODUCT_NAME}'s editing scope."
        ),
    )
    default_codeloomignore_file = branding.repo_file(git_root, branding.IGNORE_FILE)

    group.add_argument(
        "--codeloomignore",
        metavar="CODELOOMIGNORE",
        type=lambda path_str: resolve_codeloomignore_path(path_str, git_root),
        default=default_codeloomignore_file,
        dest="codeloomignore",
        help=f"Specify the {branding.PRODUCT_NAME} ignore file (default: {branding.IGNORE_FILE} in git root)",
    ).complete = shtab.FILE
    group.add_argument(
        "--subtree-only",
        action="store_true",
    ...
    group.add_argument(
        ...
        default=None,
        help=(
            f"Attribute {branding.PRODUCT_NAME} code changes in the git author name (default: True). If explicitly set"
    ...
    group.add_argument(
        ...
        default=None,
        help=(
            f"Attribute {branding.PRODUCT_NAME} commits in the git committer name (default: True). If explicitly set"
            f" to True, overrides --attribute-co-authored-by precedence for {branding.PRODUCT_NAME} edits."
        ),
    )
    group.add_argument(
        ...
        default=False,
        help=(
            f"Prefix commit messages with '{branding.COMMIT_MESSAGE_PREFIX}' if "
            f"{branding.PRODUCT_NAME} authored the changes (default: False)"
        ),
    )
    group.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=False,
        help=f"Prefix all commit messages with '{branding.COMMIT_MESSAGE_PREFIX}' (default: False)",
    )
    group.add_argument(
        ...
        default=True,
        help=(
            f"Attribute {branding.PRODUCT_NAME} edits using the Co-authored-by trailer in the commit message"
    ...
    group.add_argument(
        "--check-update",
        action=argparse.BooleanOptionalAction,
        help=f"Check for new {branding.PRODUCT_NAME} versions on launch",
        default=False,
    )
    group.add_argument(
        "--show-release-notes",
    ...
    group.add_argument(
        ...
        "--update",
        action="store_true",
        help=f"Upgrade {branding.PRODUCT_NAME} to the latest version from PyPI",
        default=False,
    )
    group.add_argument(
        "--version",
    ...
    group.add_argument(
        ...
        "--browser",
        action=argparse.BooleanOptionalAction,
        help=f"Run {branding.PRODUCT_NAME} in your browser (default: False)",
        default=False,
    )
    group.add_argument(
        "--copy-paste",
        ...
        default=False,
        help=(
            "Enable automatic copy/paste of chat between "
            f"{branding.PRODUCT_NAME} and web UI (default: False)"
        ),
    )
    group.add_argument(
        "--apply",
    ...
    group.add_argument(
        ...
        metavar="CONFIG_FILE",
        help=(
            f"Specify the config file (default: search for {branding.CONFIG_FILE} in git root, cwd"
            " or home directory)"
        ),
    ).complete = shtab.FILE
    ...
    group.add_argument(
        ...
        help=(
            ...
            f" {', '.join(supported_shells_list)}. Example: {branding.CLI_NAME} --shell-completions bash"
        ),
    )
    ...
    if command == "md":
        ...
        if len(sys.argv) > 2:
            ...
            if shell not in shtab.SUPPORTED_SHELLS:
                ...
                sys.exit(1)
            parser = get_parser([], None)
            parser.prog = branding.CLI_NAME  # Set the program name on the parser
