...
import codeloom
from codeloom import branding, utils
from codeloom.dump import dump  # noqa: F401

VERSION_CHECK_FNAME = branding.user_data_path("caches", branding.VERSION_CHECK_FILE)


def install_from_main_branch(io):
    """
    Install the latest version of Codeloom from the main branch of the GitHub repository.
    """

    return utils.check_pip_install_extra(
        io,
        None,
        f"Install the development version of {branding.PRODUCT_NAME} from the main branch?",
        [f"git+{branding.REPOSITORY_URL}.git"],
        self_update=True,
    )
...
def install_upgrade(io, latest_version=None):
    """
    Install the latest version of Codeloom from PyPI.
    """

    if latest_version:
        new_ver_text = f"Newer {branding.PRODUCT_NAME} version v{latest_version} is available."
    else:
        new_ver_text = f"Install latest version of {branding.PRODUCT_NAME}?"

    docker_image = os.environ.get("CODELOOM_DOCKER_IMAGE")
    if docker_image:
        text = f"""
...
"""
    ...
    success = utils.check_pip_install_extra(
        ...
        None,
        new_ver_text,
        [branding.PYPI_PACKAGE],
        self_update=True,
    )

    if success:
        io.tool_output(f"Re-run {branding.CLI_NAME} to use new version.")
        sys.exit()
...
def check_version(io, just_check=False, verbose=False):
    ...
    try:
        response = requests.get(f"https://pypi.org/pypi/{branding.PYPI_PACKAGE}/json")
        data = response.json()
        latest_version = data["info"]["version"]
        current_version = codeloom.__version__
