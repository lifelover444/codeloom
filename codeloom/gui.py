...
from codeloom import urls
from codeloom.coders import Coder
from codeloom.dump import dump  # noqa: F401
from codeloom.io import InputOutput
from codeloom.main import main as cli_main
from codeloom.scrape import Scraper, has_playwright
...
def search(text=None):
    results = []
    for root, _, files in os.walk("codeloom"):
        for file in files:
...
class GUI:
    ...
    def show_edit_info(self, edit):
        ...
        if commit_hash:
            ...
            if commit_hash == self.coder.last_codeloom_commit_hash:
                show_undo = True
    ...
    def do_sidebar(self):
        with st.sidebar:
            ...
            st.warning(
                "This browser version of codeloom is experimental. Please share feedback in [GitHub"
                " issues](https://github.com/Codeloom-AI/codeloom/issues)."
            )
    ...
    def do_recommended_actions(self):
        text = "Codeloom works best when your code is stored in a git repo.  \n"
        ...
        with st.expander("Recommended actions", expanded=True):
            ...
            with st.popover("Update your `.gitignore` file"):
                st.write("It's best to keep codeloom's internal files out of your git repo.")
                self.button("Add `.codeloom*` to `.gitignore`", key=random.random(), help="?")
    ...
    def do_add_files(self):
        fnames = st.multiselect(
            ...
            help=(
                ...
                " on. Codeloom will pull in other relevant code to provide context to the LLM."
            ),
        )
    ...
    def initialize_state(self):
        ...
        self.state.init("last_codeloom_commit_hash", self.coder.last_codeloom_commit_hash)
    ...
    def process_chat(self):
        ...
        with self.messages:
            edit = dict(
                role="edit",
                fnames=self.coder.codeloom_edited_files,
            )
            if self.state.last_codeloom_commit_hash != self.coder.last_codeloom_commit_hash:
                edit["commit_hash"] = self.coder.last_codeloom_commit_hash
                edit["commit_message"] = self.coder.last_codeloom_commit_message
                commits = f"{self.coder.last_codeloom_commit_hash}~1"
                diff = self.coder.repo.diff_commits(
                    self.coder.pretty,
                    commits,
                    self.coder.last_codeloom_commit_hash,
                )
                edit["diff"] = diff
                self.state.last_codeloom_commit_hash = self.coder.last_codeloom_commit_hash
    ...
    def do_undo(self, commit_hash):
        ...
        if (
            self.state.last_codeloom_commit_hash != commit_hash
            or self.coder.last_codeloom_commit_hash != commit_hash
        ):
...
def gui_main():
    st.set_page_config(
        ...
        menu_items={
            ...
            "Report a bug": "https://github.com/Codeloom-AI/codeloom/issues",
