...
from codeloom import __version__, branding, models, urls, utils
from codeloom.analytics import Analytics
from codeloom.args import get_parser
from codeloom.coders import Coder
from codeloom.coders.base_coder import UnknownEditFormat
from codeloom.commands import Commands, SwitchCoder
from codeloom.copypaste import ClipboardWatcher
from codeloom.deprecated import handle_deprecated_model_args
from codeloom.format_settings import format_settings, scrub_sensitive_info
from codeloom.history import ChatSummary
from codeloom.io import InputOutput
from codeloom.llm import litellm  # noqa: F401; properly init litellm on launch
from codeloom.models import ModelSettings
from codeloom.onboarding import offer_openrouter_oauth, select_default_model
from codeloom.repo import ANY_GIT_ERROR, GitRepo
from codeloom.report import report_uncaught_exceptions
from codeloom.versioncheck import check_version, install_from_main_branch, install_upgrade
from codeloom.watch import FileWatcher
...
def setup_git(git_root, io):
    ...
    if git_root:
        ...
        io.tool_warning(
            "You should probably run codeloom in your project's directory, not your home dir."
        )
        return
    elif cwd and io.confirm_ask(
        "No git repo found, create one to track codeloom's changes (recommended)?"
    ):
...
def check_gitignore(git_root, io, ask=True):
    ...
    try:
        ...
        if not repo.ignored(branding.USER_DIR_NAME):
            patterns_to_add.append(f"{branding.USER_DIR_NAME}*")
...
def check_streamlit_install(io):
    return utils.check_pip_install_extra(
        io,
        "streamlit",
        f"You need to install the {branding.PRODUCT_NAME} browser feature",
        [f"{branding.PYPI_PACKAGE}[browser]"],
    )
...
def write_streamlit_credentials():
    ...
    # See https://github.com/Codeloom-AI/codeloom/issues/772
...
def launch_gui(args):
    ...
    # https://github.com/Codeloom-AI/codeloom/issues/2193
...
def register_models(git_root, model_settings_fname, io, verbose=False):
    model_settings_files = generate_search_path_list(
        branding.MODEL_SETTINGS_FILE, git_root, model_settings_fname
    )
    ...
    try:
        ...
        io.tool_error(f"Error loading {branding.PRODUCT_NAME} model settings: {e}")
        return 1
...
def load_dotenv_files(git_root, dotenv_fname, encoding="utf-8"):
    ...
    oauth_keys_file = branding.user_data_path(branding.OAUTH_KEYS_FILE)
...
def register_litellm_models(git_root, model_metadata_fname, io, verbose=False):
    ...
    resource_metadata = importlib_resources.files("codeloom.resources").joinpath("model-metadata.json")
    model_metadata_files.append(str(resource_metadata))

    model_metadata_files += generate_search_path_list(
        branding.MODEL_METADATA_FILE, git_root, model_metadata_fname
    )
...
def sanity_check_repo(repo, io):
    ...
    if bad_ver:
        io.tool_error("Codeloom only works with git repos with version number 1 or 2.")
        io.tool_output("You may be able to convert your repo: git update-index --index-version=2")
        io.tool_output("Or run codeloom --no-git to proceed without using git.")
...
def main(argv=None, input=None, output=None, force_git_root=None, return_coder=False):
    ...
    conf_fname = Path(branding.CONFIG_FILE)
    ...
    if args.analytics is not False:
        if analytics.need_to_ask(args.analytics):
            io.tool_output(
                "Codeloom respects your privacy and never collects your code, chat messages, keys or"
                " personal info."
            )
            io.tool_output(f"For more info: {urls.analytics}")
            disable = not io.confirm_ask(
                "Allow collection of anonymous analytics to help improve codeloom?"
            )
    ...
    if args.codeloomignore:
        ignores.append(args.codeloomignore)
    ...
    if args.apply:
        ...
        # For testing #2879
        # from codeloom.coders.base_coder import all_fences
...
def is_first_run_of_new_version(io, verbose=False):
    ...
    installs_file = branding.user_data_path(branding.INSTALLS_FILE)
    ...
    try:
        if is_first_run:
            ...
            try:
                ...
                io.tool_error(str(err))
                io.tool_output("Error loading required imports. Did you install codeloom properly?")
