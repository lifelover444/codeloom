...
from codeloom.openrouter import OpenRouterModelManager
from codeloom.sendchat import ensure_alternating_roles, sanity_check_messages
from codeloom.utils import check_pip_install_extra
...
MODEL_SETTINGS = []
with importlib.resources.open_text("codeloom.resources", "model-settings.yml") as f:
...
class ModelInfoManager:
    ...
    def __init__(self):
        self.cache_dir = Path.home() / ".codeloom" / "caches"
...
class Model(ModelSettings):
    def __init__(
        ...
        self.extra_model_settings = next(
            (ms for ms in MODEL_SETTINGS if ms.name == "codeloom/extra_params"), None
        )
    ...
    def configure_model_settings(self, model):
        ...
        if (
            ...
            and self.extra_model_settings.name == "codeloom/extra_params"
        ):
    ...
    def send_completion(self, messages, functions, stream, temperature=None):
        if os.environ.get("CODELOOM_SANITY_CHECK_TURNS"):
        ...
        if "GITHUB_COPILOT_TOKEN" in os.environ:
            if "extra_headers" not in kwargs:
                kwargs["extra_headers"] = {
                    "Editor-Version": f"codeloom/{__version__}",
    ...
    def simple_send_with_retries(self, messages):
        ...
        while True:
            try:
                ...
                from codeloom.reasoning_tags import remove_reasoning_content
