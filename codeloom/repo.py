...
from codeloom import prompts, utils
...
class GitRepo:
    repo = None
    codeloom_ignore_file = None
    codeloom_ignore_spec = None
    codeloom_ignore_ts = 0
    codeloom_ignore_last_check = 0
    subtree_only = False
    ignore_file_cache = {}
    git_repo_error = None
    ...
    def __init__(
        ...
        if codeloom_ignore_file:
            self.codeloom_ignore_file = Path(codeloom_ignore_file)

    def commit(self, fnames=None, context=None, message=None, codeloom_edits=False, coder=None):
        """
        ...
        Args:
            ...
            codeloom_edits (bool, optional): Whether the changes were made by Codeloom. Defaults to False.
        ...
        Codeloom generated the changes (`codeloom_edits`).
        ...
        - codeloom_edits=True: Changes were generated by Codeloom (LLM).
        - codeloom_edits=False: Commit is user-driven (e.g., /commit manually staged changes).
        ...
        Flags:
        - --attribute-author: Modify Author name to "User Name (codeloom)".
        - --attribute-committer: Modify Committer name to "User Name (codeloom)".
        - --attribute-co-authored-by: Add
          "Co-authored-by: codeloom (<model>) <codeloom@codeloom.chat>" trailer to commit message.

        Behavior Summary:

        1. When codeloom_edits = True (AI Changes):
        ...
        2. When codeloom_edits = False (User Changes):
           - --attribute-co-authored-by is IGNORED (trailer never added).
           - Author name is NEVER modified (--attribute-author ignored).
           - Committer name IS modified by default (implicit True, as Codeloom runs `git commit`).
           - EXCEPTION: If --attribute-committer is EXPLICITLY False, the name is NOT modified.

        Resulting Scenarios:
        - Standard AI edit (defaults): Co-authored-by=False -> Author=You(codeloom),
          Committer=You(codeloom)
        ...
        - AI edit with Co-authored-by + Explicit Author: Co-authored-by=True,
          --attribute-author -> Author=You(codeloom), Committer=You, Trailer added
        - User commit (defaults): codeloom_edits=False -> Author=You, Committer=You(codeloom)
        - User commit with explicit no-committer: codeloom_edits=False,
        ...
        prefix_commit_message = codeloom_edits and (
        ...
        if codeloom_edits and attribute_co_authored_by:
            model_name = "unknown-model"
            if coder and hasattr(coder, "main_model") and coder.main_model.name:
                model_name = coder.main_model.name
            commit_message_trailer = f"\n\nCo-authored-by: codeloom ({model_name}) <codeloom@codeloom.chat>"

        # Determine if author/committer names should be modified
        # Author modification applies only to codeloom edits.
        # It's used if effective_author is True AND
        # (co-authored-by is False OR author was explicitly set).
        use_attribute_author = (
            codeloom_edits and effective_author and (not attribute_co_authored_by or author_explicit)
        )

        # Committer modification applies regardless of codeloom_edits (based on tests).
        # It's used if effective_committer is True AND
        # (it's not an codeloom edit with co-authored-by OR committer was explicitly set).
        use_attribute_committer = effective_committer and (
            not (codeloom_edits and attribute_co_authored_by) or committer_explicit
        )
        ...
        if prefix_commit_message:
            commit_message = "codeloom: " + commit_message
        ...
        committer_name = f"{original_user_name} (codeloom)"
    ...
    def refresh_codeloom_ignore(self):
        if not self.codeloom_ignore_file:
            return

        current_time = time.time()
        if current_time - self.codeloom_ignore_last_check < 1:
            return

        self.codeloom_ignore_last_check = current_time

        if not self.codeloom_ignore_file.is_file():
            return

        mtime = self.codeloom_ignore_file.stat().st_mtime
        if mtime != self.codeloom_ignore_ts:
            self.codeloom_ignore_ts = mtime
            self.ignore_file_cache = {}
            lines = self.codeloom_ignore_file.read_text().splitlines()
            self.codeloom_ignore_spec = pathspec.PathSpec.from_lines(
    ...
    def ignored_file_raw(self, fname):
        ...
        if not self.codeloom_ignore_file or not self.codeloom_ignore_file.is_file():
            return False
        ...
        return self.codeloom_ignore_spec.match_file(fname)
