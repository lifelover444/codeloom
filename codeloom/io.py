...
from codeloom.mdstream import MarkdownStream
...
# Constants
NOTIFICATION_MESSAGE = "Codeloom is waiting for your input"
...
class InputOutput:
    ...
    def __init__(
        ...
        input=None,
        output=None,
        user_input_color="#F2B84B",
        tool_output_color="#8A8F98",
        tool_error_color="#FF5C5C",
        tool_warning_color="#F59E0B",
        assistant_output_color="#D8D3C7",
        ...
        self.append_chat_history(f"\n# codeloom chat started at {current_time}\n\n")
    ...
    def get_default_notification_command(self):
        ...
        if system == "Darwin":  # macOS
            ...
            if shutil.which("terminal-notifier"):
                return f"terminal-notifier -title 'Codeloom' -message '{NOTIFICATION_MESSAGE}'"
            # Fall back to osascript
            return (
                f'osascript -e \'display notification "{NOTIFICATION_MESSAGE}" with title "Codeloom"\''
            )
        elif system == "Linux":
            ...
            for cmd in ["notify-send", "zenity"]:
                if shutil.which(cmd):
                    if cmd == "notify-send":
                        return f"notify-send 'Codeloom' '{NOTIFICATION_MESSAGE}'"
                    elif cmd == "zenity":
