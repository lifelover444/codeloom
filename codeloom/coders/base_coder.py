...
from codeloom import __version__, models, prompts, urls, utils
from codeloom.analytics import Analytics
from codeloom.commands import Commands
from codeloom.exceptions import LiteLLMExceptions
from codeloom.history import ChatSummary
from codeloom.io import ConfirmGroup, InputOutput
from codeloom.linter import Linter
from codeloom.llm import litellm
from codeloom.models import RETRY_TIMEOUT
from codeloom.reasoning_tags import (
    REASONING_TAG,
...
from codeloom.run_cmd import run_cmd
from codeloom.utils import format_content, format_messages, format_tokens, is_image_file
from codeloom.waiting import WaitingSpinner
...
class Coder:
    ...
    repo = None
    last_codeloom_commit_hash = None
    codeloom_edited_files = None
    ...
    def create(
        ...
        **kwargs,
        ...
        import codeloom.coders as coders
        ...
        if from_coder:
            ...
            update = dict(
                ...
                codeloom_commit_hashes=from_coder.codeloom_commit_hashes,
    ...
    def get_announcements(self):
        lines = []
        lines.append(f"Codeloom v{__version__}")
        ...
        if self.repo:
            ...
            if num_files > 1000:
                lines.append(
                    "Warning: For large repos, consider using --subtree-only and .codeloomignore"
                )
    ...
    def __init__(
        ...
        self.codeloom_commit_hashes = set()
        ...
        if codeloom_commit_hashes:
            self.codeloom_commit_hashes = codeloom_commit_hashes
        else:
            self.codeloom_commit_hashes = set()
        ...
        for fname in fnames:
            ...
            if self.repo and self.repo.ignored_file(fname):
                self.io.tool_warning(f"Skipping {fname} that matches codeloomignore spec.")
                continue
    ...
    def init_before_message(self):
        self.codeloom_edited_files = set()
    ...
    def warm_cache(self, chunks):
        ...
        delay = 5 * 60 - 5
        delay = float(os.environ.get("CODELOOM_CACHE_KEEPALIVE_DELAY", delay))
    ...
    def send_message(self, inp):
        ...
        if edited:
            self.codeloom_edited_files.update(edited)
    ...
    def auto_commit(self, edited, context=None):
        ...
        try:
            res = self.repo.commit(fnames=edited, context=context, codeloom_edits=True, coder=self)
            if res:
    ...
    def show_auto_commit_outcome(self, res):
        commit_hash, commit_message = res
        self.last_codeloom_commit_hash = commit_hash
        self.codeloom_commit_hashes.add(commit_hash)
        self.last_codeloom_commit_message = commit_message
        if self.show_diffs:
    ...
    def show_undo_hint(self):
        ...
        if self.commit_before_message[-1] != self.repo.get_head_commit_sha():
            self.io.tool_output("You can use /undo to undo and discard each codeloom commit.")
