...
from codeloom.onboarding import (
...
class TestOnboarding(unittest.TestCase):
    ...
    @patch("codeloom.onboarding.check_openrouter_tier")
    ...
    @patch("codeloom.onboarding.check_openrouter_tier", return_value=True)  # Assume free tier
    ...
    @patch("codeloom.onboarding.check_openrouter_tier", return_value=False)  # Assume paid tier
    @patch.dict(os.environ, {"OPENROUTER_API_KEY": "or_key"}, clear=True)
    ...
    @patch("codeloom.onboarding.check_openrouter_tier", return_value=False)  # Paid
    @patch.dict(
    ...
    @patch("codeloom.onboarding.try_to_select_default_model", return_value="gpt-4o")
    @patch("codeloom.onboarding.offer_openrouter_oauth")
    ...
    @patch(
        "codeloom.onboarding.try_to_select_default_model", side_effect=[None, None]
    )  # Fails first, fails after oauth attempt
    @patch(
        "codeloom.onboarding.offer_openrouter_oauth", return_value=False
    ...
    @patch(
        "codeloom.onboarding.try_to_select_default_model",
        side_effect=[None, "openrouter/deepseek/deepseek-r1:free"],
    )  # Fails first, succeeds after oauth
    @patch(
        "codeloom.onboarding.offer_openrouter_oauth", return_value=True
    ...
    @patch("codeloom.onboarding.start_openrouter_oauth_flow", return_value="new_or_key")
    ...
    @patch("codeloom.onboarding.start_openrouter_oauth_flow", return_value=None)  # OAuth fails
    @patch.dict(os.environ, {}, clear=True)
    ...
    @patch("codeloom.onboarding.start_openrouter_oauth_flow")
