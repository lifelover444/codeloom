...
from codeloom.main import main
...
class TestSSLVerification(TestCase):
    ...
    @patch("codeloom.io.InputOutput.offer_url")
    @patch("codeloom.models.ModelInfoManager.set_verify_ssl")
    @patch("codeloom.llm.litellm._load_litellm")
    @patch("httpx.Client")
    @patch("httpx.AsyncClient")
    @patch("codeloom.models.fuzzy_match_models", return_value=[])
    def test_no_verify_ssl_flag_sets_model_info_manager(
        ...
        with patch("codeloom.models.Model") as mock_model:
            ...
            with patch("codeloom.llm.litellm._lazy_module", mock_module):
    ...
    @patch("codeloom.io.InputOutput.offer_url")
    @patch("codeloom.models.model_info_manager.set_verify_ssl")
    def test_default_ssl_verification(self, mock_set_verify_ssl, mock_offer_url):
        # Prevent actual URL opening
        mock_offer_url.return_value = False
        # Run main without --no-verify-ssl flag
        with patch("codeloom.main.InputOutput"):
            with patch("codeloom.coders.Coder.create"):
