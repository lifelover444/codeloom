...
from codeloom.main import sanity_check_repo
...
def test_git_index_version_greater_than_2(mock_browser, create_repo, mock_io):
    ...
    mock_io.tool_error.assert_called_with(
        "Codeloom only works with git repos with version number 1 or 2."
    )
    ...
    mock_io.tool_output.assert_any_call("Or run codeloom --no-git to proceed without using git.")
