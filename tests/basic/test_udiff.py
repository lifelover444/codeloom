...
from codeloom.coders.udiff_coder import find_diffs
...
1. Add the `--check-update` argument to the argument parser in `codeloom/main.py`.
2. Modify the `check_version` function in `codeloom/versioncheck.py` to return a boolean indicating whether an update is available.
3. Use the returned value from `check_version` in `codeloom/main.py` to set the exit status code when `--check-update` is used.

Here are the diffs for those changes:

```diff
--- codeloom/versioncheck.py
+++ codeloom/versioncheck.py
@@ ... @@
