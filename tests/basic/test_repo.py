...
from codeloom.models import Model
from codeloom.repo import GitRepo
from codeloom.utils import GitTemporaryDirectory
...
class TestRepo(unittest.TestCase):
    ...
    @patch("codeloom.models.Model.simple_send_with_retries")
    ...
    @patch("codeloom.repo.GitRepo.get_commit_message")
    def test_commit_with_custom_committer_name(self, mock_send):
        ...
        with GitTemporaryDirectory():
            ...
            # commit a change with codeloom_edits=True (using default attributes)
            fname.write_text("new content")
            commit_result = git_repo.commit(fnames=[str(fname)], codeloom_edits=True)
            self.assertIsNotNone(commit_result)

            # check the committer name (defaults interpreted as True)
            commit = raw_repo.head.commit
            self.assertEqual(commit.author.name, "Test User (codeloom)")
            self.assertEqual(commit.committer.name, "Test User (codeloom)")

            # commit a change without codeloom_edits (using default attributes)
            fname.write_text("new content again!")
            commit_result = git_repo.commit(fnames=[str(fname)], codeloom_edits=False)
            self.assertIsNotNone(commit_result)
            ...
            commit_result = git_repo_explicit_false.commit(fnames=[str(fname)], codeloom_edits=True)
            self.assertIsNotNone(commit_result)
            commit = raw_repo.head.commit
            ...
            commit_result = git_repo_user_no_committer.commit(
                fnames=[str(fname)], codeloom_edits=False
            )
            self.assertIsNotNone(commit_result)
            commit = raw_repo.head.commit
            self.assertEqual(
    ...
    def test_commit_with_co_authored_by(self):
        with GitTemporaryDirectory():
            ...
            # commit a change with codeloom_edits=True and co-authored-by flag
            fname.write_text("new content")
            commit_result = git_repo.commit(
                fnames=[str(fname)], codeloom_edits=True, coder=mock_coder, message="Codeloom edit"
            )
            self.assertIsNotNone(commit_result)

            # check the commit message and author/committer
            commit = raw_repo.head.commit
            self.assertIn("Co-authored-by: codeloom (gpt-test) <codeloom@codeloom.chat>", commit.message)
            self.assertEqual(commit.message.splitlines()[0], "Codeloom edit")
    ...
    def test_commit_co_authored_by_with_explicit_name_modification(self):
        ...
        with GitTemporaryDirectory():
            ...
            # commit a change with codeloom_edits=True and combo flags
            fname.write_text("new content combo")
            commit_result = git_repo.commit(
                fnames=[str(fname)], codeloom_edits=True, coder=mock_coder, message="Codeloom combo edit"
            )
            self.assertIsNotNone(commit_result)
            ...
            commit = raw_repo.head.commit
            self.assertIn(
                "Co-authored-by: codeloom (gpt-test-combo) <codeloom@codeloom.chat>", commit.message
            )
            self.assertEqual(commit.message.splitlines()[0], "Codeloom combo edit")
    ...
    def test_commit_ai_edits_no_coauthor_explicit_false(self):
        # Test AI edits (codeloom_edits=True) when co-authored-by is False,
        ...
        with GitTemporaryDirectory():
            ...
            commit_result = git_repo_no_author.commit(
                ...
                message="Codeloom no author",
            )
            self.assertIsNotNone(commit_result)
            commit = raw_repo.head.commit
            self.assertNotIn("Co-authored-by:", commit.message)
            self.assertEqual(commit.author.name, "Test User")  # Explicit False
            self.assertEqual(commit.committer.name, "Test User (codeloom)")  # Default True
    ...
    def test_get_tracked_files_with_codeloomignore(self):
        with GitTemporaryDirectory():
            # new repo
            raw_repo = git.Repo()
            ...
            codeloomignore = Path(".codeloomignore")
            git_repo = GitRepo(InputOutput(), None, None, str(codeloomignore))
            ...
            codeloomignore.write_text("new.txt\n")
            time.sleep(2)
            ...
            #
            # codeloomignore.write_text("new2.txt\n")
