...
from codeloom.coders import Coder
from codeloom.dump import dump  # noqa: F401
from codeloom.io import InputOutput
from codeloom.main import check_gitignore, load_dotenv_files, main, setup_git
from codeloom.utils import GitTemporaryDirectory, IgnorantTemporaryDirectory, make_repo
...
class TestMain(TestCase):
    def setUp(self):
        ...
        os.environ["CODELOOM_CHECK_UPDATE"] = "false"
        os.environ["CODELOOM_ANALYTICS"] = "false"
        self.original_cwd = os.getcwd()
        self.tempdir_obj = IgnorantTemporaryDirectory()
        self.tempdir = self.tempdir_obj.name
        os.chdir(self.tempdir)
        # Fake home directory prevents tests from using the real ~/.codeloom.conf.yml file:
        self.homedir_obj = IgnorantTemporaryDirectory()
        os.environ["HOME"] = self.homedir_obj.name
        self.input_patcher = patch("builtins.input", return_value=None)
        self.mock_input = self.input_patcher.start()
        self.webbrowser_patcher = patch("codeloom.io.webbrowser.open")
    ...
    @patch("codeloom.repo.GitRepo.get_commit_message", return_value="mock commit message")
    ...
    def test_main_with_git_config_yml(self):
        ...
        Path(".codeloom.conf.yml").write_text("auto-commits: false\n")
        with patch("codeloom.coders.Coder.create") as MockCoder:
            main(["--yes"], input=DummyInput(), output=DummyOutput())
            _, kwargs = MockCoder.call_args
            assert kwargs["auto_commits"] is False

        Path(".codeloom.conf.yml").write_text("auto-commits: true\n")
        with patch("codeloom.coders.Coder.create") as MockCoder:
    ...
    def test_main_with_empty_git_dir_new_subdir_file(self):
        ...
        # Because codeloom will try and `git add` a file that's already in the repo.
    ...
    def test_setup_git(self):
        ...
        self.assertEqual(".codeloom*", gitignore.read_text().splitlines()[0])
    ...
    def test_encodings_arg(self):
        ...
        with GitTemporaryDirectory():
            with patch("codeloom.coders.Coder.create") as MockCoder:  # noqa: F841
                with patch("codeloom.main.InputOutput") as MockSend:
    ...
    def test_main_exit_calls_version_check(self):
        with GitTemporaryDirectory():
            with (
                patch("codeloom.main.check_version") as mock_check_version,
                patch("codeloom.main.InputOutput") as mock_input_output,
            ):
                main(["--exit", "--check-update"], input=DummyInput(), output=DummyOutput())
                mock_check_version.assert_called_once()
                mock_input_output.assert_called_once()

    @patch("codeloom.main.InputOutput")
    @patch("codeloom.coders.base_coder.Coder.run")
    ...
    def test_env_file_flag_sets_automatic_variable(self):
        env_file_path = self.create_env_file(".env.test", "CODELOOM_DARK_MODE=True")
        with patch("codeloom.main.InputOutput") as MockInputOutput:
            MockInputOutput.return_value.get_input.return_value = None
    ...
    def test_true_vals_in_env_file(self):
        self.create_env_file(".env", "CODELOOM_SHOW_DIFFS=on")
        with patch("codeloom.coders.Coder.create") as MockCoder:
            main(["--no-git", "--yes"], input=DummyInput(), output=DummyOutput())
            MockCoder.assert_called_once()
            _, kwargs = MockCoder.call_args
    ...
    def test_lint_option(self):
        with GitTemporaryDirectory() as git_dir:
            ...
            with patch("codeloom.linter.Linter.lint") as MockLinter:
    ...
    def test_verbose_mode_lists_env_vars(self):
        self.create_env_file(".env", "CODELOOM_DARK_MODE=on")
        with patch("sys.stdout", new_callable=StringIO) as mock_stdout:
            ...
            relevant_output = "\n".join(
                ...
                if "CODELOOM_DARK_MODE" in line or "dark_mode" in line
            )  # this bit just helps failing assertions to be easier to read
            self.assertIn("CODELOOM_DARK_MODE", relevant_output)
            self.assertIn("dark_mode", relevant_output)
            self.assertRegex(relevant_output, r"CODELOOM_DARK_MODE:\s+on")
    ...
    def test_yaml_config_file_loading(self):
        with GitTemporaryDirectory() as git_dir:
            ...
            # Create .codeloom.conf.yml files in different locations
            home_config = fake_home / ".codeloom.conf.yml"
            git_config = git_dir / ".codeloom.conf.yml"
            cwd_config = cwd / ".codeloom.conf.yml"
            named_config = git_dir / "named.codeloom.conf.yml"
            ...
            with (
                ...
                patch("codeloom.coders.Coder.create") as MockCoder,
            ):
    ...
    def test_map_tokens_option(self):
        with GitTemporaryDirectory():
            with patch("codeloom.coders.base_coder.RepoMap") as MockRepoMap:
    ...
    def test_model_metadata_file(self):
        ...
        from codeloom import models

        models.model_info_manager = models.ModelInfoManager()

        from codeloom.llm import litellm

        litellm._lazy_module = None

        with GitTemporaryDirectory():
            metadata_file = Path(".codeloom.model.metadata.json")
    ...
    def test_accepts_settings_warnings(self):
        ...
        with GitTemporaryDirectory():
            ...
            with (
                patch("codeloom.io.InputOutput.tool_warning") as mock_warning,
                patch("codeloom.models.Model.set_thinking_tokens") as mock_set_thinking,
            ):
                main(
                    [
                        "--model",
            ...
            with (
                patch("codeloom.io.InputOutput.tool_warning") as mock_warning,
                patch("codeloom.models.Model.set_reasoning_effort") as mock_set_reasoning,
            ):
                main(
    ...
    @patch("codeloom.models.ModelInfoManager.set_verify_ssl")
    def test_no_verify_ssl_sets_model_info_manager(self, mock_set_verify_ssl):
        with GitTemporaryDirectory():
            ...
            with patch("codeloom.models.Model") as mock_model:
                ...
                with patch("codeloom.models.fuzzy_match_models", return_value=[]):
                    main(
    ...
    def test_pytest_env_vars(self):
        ...
        self.assertEqual(os.environ.get("CODELOOM_ANALYTICS"), "false")
    ...
    def test_git_config_include(self):
        # Test that codeloom respects git config includes for user.name and user.email
        with GitTemporaryDirectory() as git_dir:
            git_dir = Path(git_dir)
            ...
            # Run codeloom and verify it doesn't change the git config
            main(["--yes", "--exit"], input=DummyInput(), output=DummyOutput())
    ...
    def test_git_config_include_directive(self):
        # Test that codeloom respects the include directive in git config
        with GitTemporaryDirectory() as git_dir:
            git_dir = Path(git_dir)
            ...
            config_after_codeloom = git_config.read_text()
            self.assertEqual(modified_config_content, config_after_codeloom)
    ...
    def test_resolve_codeloomignore_path(self):
        # Import the function directly to test it
        from codeloom.args import resolve_codeloomignore_path

        # Test with absolute path
        abs_path = os.path.abspath("/tmp/test/.codeloomignore")
        self.assertEqual(resolve_codeloomignore_path(abs_path), abs_path)

        # Test with relative path and git root
        git_root = "/path/to/git/root"
        rel_path = ".codeloomignore"
        self.assertEqual(
            resolve_codeloomignore_path(rel_path, git_root), str(Path(git_root) / rel_path)
        )

        # Test with relative path and no git root
        rel_path = ".codeloomignore"
        self.assertEqual(resolve_codeloomignore_path(rel_path), rel_path)
    ...
    def test_default_model_selection(self):
        with GitTemporaryDirectory():
            ...
            with patch("codeloom.onboarding.offer_openrouter_oauth") as mock_offer_oauth:
    ...
    def test_check_model_accepts_settings_flag(self):
        ...
        with GitTemporaryDirectory():
            ...
            with patch("codeloom.models.Model.set_thinking_tokens") as mock_set_thinking:
                main(
                    [
                        "--model",
                        "gpt-4o",
    ...
    def test_list_models_with_direct_resource_patch(self):
        ...
        with GitTemporaryDirectory():
            ...
            with patch("codeloom.main.importlib_resources.files", return_value=mock_files):
                # Capture stdout to check the output
                with patch("sys.stdout", new_callable=StringIO) as mock_stdout:
                    main(
            ...
            with patch("codeloom.models.Model.set_reasoning_effort") as mock_set_reasoning:
                main(
                    [
                        "--model",
                        "gpt-3.5-turbo",
    ...
    def test_model_accepts_settings_attribute(self):
        with GitTemporaryDirectory():
            ...
            with patch("codeloom.models.Model") as MockModel:
    ...
    def test_load_dotenv_files_override(self):
        with GitTemporaryDirectory() as git_dir:
            ...
            # Create fake home and .codeloom directory
            fake_home = git_dir / "fake_home"
            fake_home.mkdir()
            codeloom_dir = fake_home / ".codeloom"
            codeloom_dir.mkdir()

            # Create oauth keys file
            oauth_keys_file = codeloom_dir / "oauth-keys.env"
