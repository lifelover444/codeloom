...
from codeloom.voice import SoundDeviceError, Voice
...
@pytest.fixture
def mock_soundfile():
    with patch("codeloom.voice.sf") as mock_sf:
        yield mock_sf
...
def test_voice_init_invalid_format():
    with patch("codeloom.voice.sf", MagicMock()):  # Need to mock sf to avoid SoundDeviceError
        with pytest.raises(ValueError) as exc:
...
def test_record_and_transcribe_keyboard_interrupt():
    with patch("codeloom.voice.sf", MagicMock()):
        voice = Voice()
